...
def Signup(request):
    ...
    if request.method == 'POST':
        ...
        if 'image' in request.FILES:
            ...
            i = None  # Set to None or a default image if required
        # Create customer record
        sign = Customer.objects.create(status=sta,user=user,mobile=con,image=i,gender=gen)
        error = True
    d = {'error':error}
...
def Edit_Profile(request):
    ...
    if request.method == 'POST':
        ...
        user.last_name = l
        if 'image' in request.FILES and request.FILES['image']:
            pro.image = request.FILES['image']  # Set the new image if uploaded
        
        pro.mobile = con  # Update the contact number
...
def Book_Appointment(request):
    ...
    current_date = date.today().isoformat()  # Example: '2024-10-25'
    ...
    if request.method == 'POST':
        ...
        error = True
    d = {'error':error,'pro':pro,'service':service,'current_date':current_date}
...
def payment(request,pid):
    ...
    if request.method == "POST":
        ...
        if PaymentCard.objects.filter(card_number=card_number).exists():
            error_message = "This card number has already been used. Please use a different card."
            return render(request, 'payment.html', {'book': book, 'error_message': error_message})
        
        # If the card number is unique, save it and proceed with payment
        PaymentCard.objects.create(user=user, card_number=card_number)
...
def Change_Password(request):
    ...
    # return render(request,'change_password.html',d)
        ...
        if new_password != confirm_password:
            error = "not"
        elif not re.match(r'^(?=.*[0-9])(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{5,}$', new_password):
            error = "invalid"
